...
public class UsageStatsUtil {
    ...
    public void setUsageStats(WingifyInitOptions options) {
        ...
        // Handle _vwo_meta
        Object wingifyMeta = options.getWingifyMetaData();
        if (wingifyMeta != null && wingifyMeta instanceof Map) {
            Map<?, ?> wingifyMetaMap = (Map<?, ?>) wingifyMeta;
            if (wingifyMetaMap.containsKey("ea")) {
                data.put("_ea", 1);
            }
        }
