...
public class LogTransportManager extends Logger implements LogTransport {
    ...
    private void invokeLogHandler(Object logHandler, LogLevelEnum level, String message) {
        if (logHandler instanceof LogTransport) {
            ((LogTransport) logHandler).log(level, message);
        } else if (logHandler instanceof com.vwo.interfaces.logger.LogTransport) {
            ((com.vwo.interfaces.logger.LogTransport) logHandler).log(
                    com.vwo.packages.logger.enums.LogLevelEnum.fromWingify(level), message);
        }
    }
    ...
    public void log(LogLevelEnum level, String message) {
        for (Map<String, Object> transport : transports) {
            ...
            if (shouldLog(level.name(), transport.get("level"))) {
                ...
                if (logHandler != null) {
                    invokeLogHandler(logHandler, level, message);
                } else {
                    Object defaultLogHandler = transport.get("defaultTransport");
                    invokeLogHandler(defaultLogHandler, level, formattedMessage);
                }
            }
        }
    }
