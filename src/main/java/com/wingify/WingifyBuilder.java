...
import com.wingify.utils.UsageStatsUtil;
...
public class WingifyBuilder {
    private WingifyClient wingifyClient;
    public WingifyInitOptions options;
    ...
    public void setWingifyClient(WingifyClient wingifyClient) {
    ...
    public WingifyBuilder setLogger() {
        try {
            loggerService = new LoggerService(createLoggerConfig());
            loggerService.log(LogLevelEnum.DEBUG, "SERVICE_INITIALIZED", new HashMap<String, Object>() {
                {
                    put("brand", getBrand(options.getIsViaVWO()));
                    put("service", "Logger");
    ...
    private Map<String, Object> createLoggerConfig() {
        Map<String, Object> config = (this.options != null && this.options.getLogger() != null)
                ? new HashMap<>(this.options.getLogger())
                : new HashMap<>();
        if (this.options != null) {
            config.putIfAbsent("isViaVWO", this.options.getIsViaVWO());
        }
        return config;
    }
    ...
    public void updatePollIntervalAndCheckAndPoll(String settings, boolean shouldCheckAndPoll) {
        ...
        if (settings != null && !settings.isEmpty()) {
            try {
                processedSettings = WingifyClient.objectMapper.readValue(settings, Settings.class);
        ...
        while (true) {
            try {
                ...
                if (originalSettings != null && latestSettings != null) {
                    JsonNode latestSettingJsonNode = WingifyClient.objectMapper.readTree(latestSettings);
                    JsonNode originalSettingsJsonNode = WingifyClient.objectMapper.readTree(originalSettings);
                    if (!latestSettingJsonNode.equals(originalSettingsJsonNode)) {
                        updateSettingsOnBuilder(latestSettings);
                    } else {
                        loggerService.log(LogLevelEnum.INFO, "POLLING_NO_CHANGE_IN_SETTINGS", new HashMap<String, Object>() {
                            {
                                put("brand", getBrand(options.getIsViaVWO()));
                            }
                        });
    ...
    /**
     * This method is used to update the settings on the WingifyBuilder instance
    ...
    private void updateSettingsOnBuilder(String latestSettings) {
        try {
            if (wingifyClient != null) {
                String updatedSettings = wingifyClient.updateSettings(latestSettings);
                if (updatedSettings != null) {
                    loggerService.log(LogLevelEnum.INFO, "POLLING_SET_SETTINGS", new HashMap<String, Object>() {
                        {
                            put("brand", getBrand(options.getIsViaVWO()));
                        }
                    });
    ...
    public WingifyBuilder initBatching() {
        ...
        if (this.options.getBatchEventData() != null) {
            ...
            this.batchEventQueue = batchEventQueue; // Link to the wingifyClient
