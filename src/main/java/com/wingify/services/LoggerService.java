...
public class LoggerService {
     ...
     * @param key The key of the message
     * @param map The map of the message
     * @param shouldLogToWingify Whether to log to VWO
     */
    public void log(LogLevelEnum level, String key, Map<String, Object> map, Boolean shouldLogToWingify) {
        switch (level) {
            case DEBUG:
            ...
            default:
                ...
                if (shouldLogToWingify) {
                    errorLogToWingify(key, message, map);
                }
        }
    }
    ...
    private Map<String, String> readLogFiles(String fileName) {
        try {
            ...
            return WingifyClient.objectMapper.readValue(inputStream, Map.class);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
     ...
     */
    private void errorLogToWingify(String template, String message, Map<String, Object> debugProps) {
        ...
        DebuggerServiceUtil.sendDebugEventToWingify(settingsManager, debugProps);
    }
}